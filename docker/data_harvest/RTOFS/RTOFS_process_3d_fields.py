...
def process_3d_fields(data_url, forecast_time, model_field_indx, levels):
    """
    process_3d_fields(data_url, forecast_time, model_field_indx, levels)
    ...
    u_comp_url = data_url;
    v_comp_url = data_url.replace('uvel','vvel')

    with get_opendapp_netcdf(u_comp_url) as file_u, get_opendapp_netcdf(v_comp_url) as file_v:
        
        formatted_folder_date = datetime.datetime.strftime(forecast_time,'%Y%m%d_%H')
         
        logger.info('processing RTOFS_DATA data: {}'.format(formatted_folder_date))

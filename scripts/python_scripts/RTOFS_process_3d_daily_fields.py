...
def lambda_handler(event, context):
    ...
    output_lat_array = np.arange(int(min(lat)), int(max(lat)) + 0.5,
                                 0.5)  # last point is excluded with arange (90 to -90)
    ...
    output_data = [
        {'header': {
            ...
            'data': [float('{:.3f}'.format(el)) if np.abs(el) > 0.0001 else 0 for el in
                     u_data_interp[::-1].flatten().tolist()]
        },
        {'header': {
            'parameterUnit': "m.s-1",
            'parameterNumber': 3,
            ...
            'data': [float('{:.3f}'.format(el)) if np.abs(el) > 0.0001 else 0 for el in
                     v_data_interp[::-1].flatten().tolist()]
        },
    ]
...
if __name__ == "__main__":
    event = {
        'url': 'http://nomads.ncep.noaa.gov:80/dods/rtofs/rtofs_global20221021/rtofs_glo_3dz_forecast_daily_uvel',
        'forecast_time': '20221021T00:00',
        'forecast_indx': 1,

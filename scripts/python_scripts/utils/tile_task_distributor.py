...
def tile_task_distributor(pickle_filepath, data_type, bucket_name, output_tilepath, zoom_array=range(4,5)):
    """
    ...
    tiles = mercantile.tiles(west=lon.min(), south=lat.min(), east=lon.max(), north=lat.max(), zooms=zoom_array)

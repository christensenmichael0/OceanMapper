...
def lambda_handler(event, context):
    ...
    required_query_params = ['time','model','level']
    ...
    dataset_type = datasets[model]['data_type']
    scalar_tiles = datasets[model]['scalar_tiles']
    vector_tiles = datasets[model]['vector_tiles']
    ...
    if available_time:
        ...
        data_key, tile_keys = build_file_path(dataset_folder, model, available_time, dataset_type, 
            scalar_tiles, vector_tiles, level_formatted)

        # get the file (if we arent dealing with waves)
        if not model == 'ww3_data':
            try:

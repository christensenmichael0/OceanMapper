...
def lambda_handler(event, context):
        ...
        for dataset in datasets:
            dataset_value = datasets[dataset]
            availability_struct[dataset_value] = {}
            filtered_objects = data_bucket.objects.filter(Prefix=dataset_value)

            for object in filtered_objects:
                
                pattern = r'model_folder/(\d{8}_\d{2})/?(\w*)/(json|pickle)/.*[.](json|pickle)$'.replace(
                'model_folder',dataset_value)

                match = re.search(pattern,object.key)
                if match:
                    parsed_date = match.group(1)
                    parsed_level = match.group(2)
                    parsed_type = match.group(3)

                    availability_struct[dataset_value].setdefault('level',{})                    
                    availability_struct[dataset_value]['level'].setdefault(parsed_level,{})
                    availability_struct[dataset_value]['level'][parsed_level].setdefault(parsed_type,[]).append(parsed_date)
